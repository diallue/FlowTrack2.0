const API = {
  listActivities: (perPage = 30, page = 1, filters = {}) => {
    ...
    return fetch(`./api/activities?${params.toString()}`, { credentials: 'same-origin' }).then(checkJson);
  }
};
...
function renderActivities(list, append = true) {
  ...
  list.forEach(act => {
    ...
    meta.innerHTML = `<div><span class="badge">${act.type}</span> ${dateShort(act.start_date_local)}</div>
                      <div>${formatDistance(act.distance || 0)} • <span class="muted">${formatTime(act.elapsed_time || 0)}</span></div>`;
...
async function loadNextPage() {
  ...
  try {
    ...
    if (activities.length === 0) {
      ...
      state.activities.push(...activities); // solo añadir los nuevos
      state.page += 1; // incrementar página
    }
...
function applyFilters() {
  state.filters = {
    type: el.filterType.value || null,
    date_from: el.filterFrom.value || null,
    date_to: el.filterTo.value || null,
    distance_min: el.filterDistance.value ? parseFloat(el.filterDistance.value) : null,
    q: el.filterSearch.value.trim() || null
  };
  reloadAll();
}

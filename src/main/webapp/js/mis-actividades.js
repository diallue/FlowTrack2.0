...
function renderActivities(list, append=true){
  ...
  const existingIds = new Set(Array.from(el.activities.querySelectorAll('.activity-card')).map(c=>c.dataset.id));

  list.forEach(act=>{
    if(existingIds.has(String(act.id))) return; // saltar duplicados
...
async function loadNextPage(){
  ...
  try {
    ...
    if (activities.length===0){ 
      ...
      // Añadir solo actividades nuevas al estado
      activities.forEach(a=>{
        if(!state.activities.find(existing=>existing.id===a.id)) state.activities.push(a);
      });
      state.page+=1; 
    }
